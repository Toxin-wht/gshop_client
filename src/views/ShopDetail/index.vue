          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="spuSaleAttr in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                @click="choose(spuSaleAttrValue,spuSaleAttr.spuSaleAttrValueList)"
                >{{spuSaleAttrValue.saleAttrValueName}}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="skuNo" @change="skuNo>1?skuNo=parseInt(skuNo):skuNo=1">
                <a href="javascript:" class="plus" @click="skuNo++">+</a>
                <a href="javascript:" class="mins" @click="skuNo<=1?skuNo=1:skuNo--">-</a>
              </div>
              <div class="add">
...
<script>
  ...
  export default {
    ...
    methods:{
      getDetialList(){
        this.$store.dispatch('reqDetialList',this.skuId)
      },
      choose(value,valueList){
      ...
      async addShopCart(){
        ...
        try {
          await this.$store.dispatch('addCart',{skuId,skuNo})
          sessionStorage.setItem('SHOPCART_KEY',JSON.stringify(this.skuInfo))
          this.$router.push('/addcart?skuNo='+skuNo)
        } catch (error) {
          alert(error.message)
        }
      }
